...
class W25X40CL : public mbed::BlockDevice {
private:
static std::unique_ptr<W25X40CL> _singleton;
public:

    static W25X40CL & getInstance();

    void deselectAllSpiCs();
    ...
    int read_unique_id(uint64_t* buffer);
...
protected:
    ...
    /** Creates a W25X40CL on a SPI bus specified by configuration
     *
     */
    W25X40CL();
    
    /** Creates a W25X40CL on a SPI bus specified by pins
     *
     ...
     */
    W25X40CL(PinName mosi, PinName miso, PinName sclk, PinName csel, int freq = 40000000);
